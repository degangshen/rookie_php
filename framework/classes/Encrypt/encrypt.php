...
class Rookie_Encrypt {
	...
	public static function instance($name = NULL)
	{
		if ($name === NULL)
		{
			...
			$name = Rookie_Encrypt::$default;
		}

		if ( ! isset(Rookie_Encrypt::$instances[$name]))
		{
			// Load the configuration data
			global $C;
			$sys_config = $C['system']['config'];
			$config['key'] = $sys_config['encrypt'];

			if ( ! isset($config['key']))
			{
				// No default encryption key is provided!
				throw new Rookie_Exception('No encryption key is defined in the encryption configuration group: :group',
					array(':group' => $name));
			}
			...
			Rookie_Encrypt::$instances[$name] = new Rookie_Encrypt($config['key'], $config['mode'], $config['cipher']);
		}

		return Rookie_Encrypt::$instances[$name];
	}
	...
	public function encode($data)
	{
		...
		if (Rookie_Encrypt::$_rand === NULL)
		{
			if (Rookie_Core::$is_windows)
			{
				// Windows only supports the system random number generator
				Rookie_Encrypt::$_rand = MCRYPT_RAND;
			}
			else
			{
				...
				{
					// Use /dev/urandom
					Rookie_Encrypt::$_rand = MCRYPT_DEV_URANDOM;
				}
				elseif (defined('MCRYPT_DEV_RANDOM'))
				{
					// Use /dev/random
					Rookie_Encrypt::$_rand = MCRYPT_DEV_RANDOM;
				}
				else
				{
		...
		if (Rookie_Encrypt::$_rand === MCRYPT_RAND)
		{
		...
		$iv = mcrypt_create_iv($this->_iv_size, Rookie_Encrypt::$_rand);
