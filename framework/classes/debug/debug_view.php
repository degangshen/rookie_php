...
<div id="kohana_error">
	...
	<div id="<?php echo $error_id ?>" class="content">
		<p>
			<span class="file"><?php echo Rookie_Debug::path($file) ?> [ <?php echo $line ?> ]</span>
		</p>
		<?php echo Rookie_Debug::source($file, $line) ?>
		<ol class="trace">
		<?php foreach (Rookie_Debug::trace($trace) as $i => $step): ?>
			<li>
				<p>
					<span class="file">
						<?php if ($step['file']): $source_id = $error_id.'source'.$i; ?>
							<a href="#<?php echo $source_id ?>" onclick="return koggle('<?php echo $source_id ?>')"><?php echo Rookie_Debug::path($step['file']) ?> [ <?php echo $step['line'] ?> ]</a>
						<?php else: ?>
				...
				<div id="<?php echo $args_id ?>" class="collapsed">
					...
					<?php foreach ($step['args'] as $name => $arg): ?>
						<tr>
							...
							<td><pre><?php echo Rookie_Debug::dump($arg) ?></pre></td>
						</tr>
					<?php endforeach ?>
					</table>
				</div>
	...
	<div id="<?php echo $env_id ?>" class="content collapsed">
		...
		<div id="<?php echo $env_id ?>" class="collapsed">
			<table cellspacing="0">
				...
				<tr>
					<td><code><?php echo Rookie_Debug::path($file) ?></code></td>
				</tr>
				<?php endforeach ?>
			</table>
		</div>
		...
		<div id="<?php echo $env_id ?>" class="collapsed">
			<table cellspacing="0">
				...
				<tr>
					...
					<td><pre><?php echo Rookie_Debug::dump($value) ?></pre></td>
				</tr>
				<?php endforeach ?>
			</table>
		</div>
