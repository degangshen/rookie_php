...
class Rookie_Form {
	...
	public static function editor($textareaid = 'content', $toolbar = 'basic', $module = '', $catid = '', $color = '', $allowupload = 0, $allowbrowser = 1,$alowuploadexts = '',$height = 200,$disabled_page = 0, $allowuploadnum = '10') {
		...
		if(!defined('EDITOR_INIT')) {
			$str = '<script type="text/javascript" src="'.JSPATH.'ckeditor/ckeditor.js"></script>';
		...
		if(!defined('IMAGES_INIT')) {
			$ext_str .= '<script type="text/javascript" src="'.JSPATH.'swfupload/swf2ckeditor.js"></script>';
			define('IMAGES_INIT', 1);
		}
		$ext_str .= "<div id='page_title_div'>
		<table cellpadding='0' cellspacing='1' border='0'><tr><td class='title'>".'提交'."<span id='msg_page_title_value'></span></td><td>
	...
	public static function date($name, $value = '', $isdatetime = 0, $loadjs = 0, $showweek = 'true') {
		...
		if($loadjs || !defined('CALENDAR_INIT')) {
			...
			$str .= '<link rel="stylesheet" type="text/css" href="'.js_path_return('calendar/jscal2.css').'"/>
			<link rel="stylesheet" type="text/css" href="'.js_path_return('calendar/border-radius.css').'"/>
			<link rel="stylesheet" type="text/css" href="'.js_path_return('calendar/win2k.css').'"/>
			<script type="text/javascript" src="'.js_path_return('calendar/calendar.js').'"></script>
			<script type="text/javascript" src="'.js_path_return('calendar/lang/en.js').'"></script>';
		}
	 ...
	 */
	public static function select_category($file = '',$cat_id = 0, $str = '', $default_option = '', $model_id = 0, $type = -1, $onlysub = 0,$site_id = 0) {
		$tree = Rookie_Core::load_help("tree");
		$tree = new tree();
		...
		//}
		Rookie_Cache_Php::$_directory = CACHEPATH . 'category_caches' . DIRECTORY_SEPARATOR;
		//读取缓存
		$result = Rookie_Cache_Php::get('category_content');
	 ...
	 *
	 * @access  public
	 * @param   int     $cat_array     分类编号
	 * @param   int     $product_id   商品编号
	 * @return  string
	 */
	public static function build_attr_html($cat_array, $product_id = 0)
	{
	    $attr = $cat_array;
	    	
	    // $html = '<table width="90%" id="attrTable">';
	    $html = '';
	    $spec = 0;
	    foreach ($attr AS $key => $val)
	    {
	    	if (!isset($val['attr_value']))
	    		$val['attr_value'] = $val['attr_values'];
	        ...
	        $html .= "$val[attr_name]</td><td><input type='hidden'  class='input-text' name='attr_id_list[]' value='$val[attr_id]' />";
	
	        if ($val['attr_input_type'] == 0)
	        {
	            $html .= '<input name="attr_values_list[]"  class="input-text" type="text" value="' .htmlspecialchars($val['attr_value']). '" size="40" /> ';
	        }
	        elseif ($val['attr_input_type'] == 2)
	        {
	            $html .= '<textarea name="attr_values_list[]" rows="3" cols="40">' .htmlspecialchars($val['attr_value']). '</textarea>';
	        }
	        else
	        {
	            $html .= '<select name="attr_values_list[]">';
			...
			$val['attr_price'] = isset($val['attr_price']) && !empty($val['attr_price']) ? $val['attr_price']: '';
			
	        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
	            ' 属性价格 <input type="text"  class="input-text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />' :
	...
	public static function checkcode($id = 'checkcode',$code_len = 4, $font_size = 20, $width = 130, $height = 50, $font = '', $font_color = '', $background = '') {
		return "<img id='$id' onclick='this.src=this.src+\"&\"+Math.random()' src='".WEBURL."api/checkcode/api.html?code_len=$code_len&font_size=$font_size&width=$width&height=$height&font_color=".urlencode($font_color)."&background=".urlencode($background)."'>";
	}
	/**
	 * url  规则调用
	 * 
