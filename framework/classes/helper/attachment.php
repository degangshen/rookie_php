...
class attachment 
{
	...
	function __construct($module='', $catid = 0,$siteid = 0,$upload_dir = '') {
		...
		Rookie_Core::load_sys_func('dir');
		
		Rookie_Core::load_help('image');
	...
	function upload($field, $alowexts = '', $maxsize = 0, $overwrite = 0,$thumb_setting = array(), $watermark_enable = 1) {
		...
		if(empty($alowexts) || $alowexts == '') {
			$site_setting = $this->_get_site_setting(1);
		...
		foreach($uploadfiles as $k=>$file) {
			...
			if(@$upload_func($file['tmp_name'], $savefile)) {
				...
				$thumb_enable = !empty($thumb_setting) && is_array($thumb_setting) && ($thumb_setting[0] > 0 || $thumb_setting[1] > 0 ) ? 1 : 0;	
	...
	function add($uploadedfile) {
		Rookie_Core::load_model('AttachmentModel', 'attachment');
		$this->att_db = new AttachmentModel();
	...
	*/
	...
	private function _get_site_setting($siteid) {
		Rookie_Cache_Php::$_directory = CACHEPATH . 'configs' . DIRECTORY_SEPARATOR;
		return Rookie_Cache_Php::get("attachment_config");
	}
}
?>