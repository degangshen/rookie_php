...
class tree 
{
	...
	*/
	public function init($arr=array()){
       $this->arr = $arr;
	   $this->ret = '';
	   include_once WEBPATH . 'modules/manage/model/CategoryModel.php';
	...
	*/
	 ...
	 * 获取栏目点击数
	 */
	public function get_category_hits($cate_id = NULL)
	{
		$res = $this->category_model->fetchAll("select * from category_hits where category_id = $cate_id");
	...
	*/
	public function get_child($myid)
	{
		...
		{
			$val['click_num'] = $this->get_category_hits($val['cat_id']);
			$new_arr[$val['cat_id']] = $val;
		}
		$this->arr = $new_arr;
		$a = $newarr = array();
		if(is_array($this->arr)){
	...
	* @return string
	*/
	public function get_tree1($myid = 0){
		$number=1;
		$child = $this->get_child($myid);
		$catInfo = $this->arr;
		foreach ($child as $key => $val)
		{
			echo ' <li class="exd"><a href="/product/search?category=women_shoes&gender=2" title="女鞋" class="exable" ><span class="ico_woman"></span></a>';
			$childCat = explode(',', $val['arr_child_id']);
			foreach ($childCat as $k => $v)
			{
				if(isset($catInfo[$v])){
				foreach ($catInfo[$v] as $key => $val)
				{
					echo $val['cat_name'];
				} 
				}
