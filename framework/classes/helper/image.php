...
class image {
    ...
    function __construct($thumb_enable = 0 ,$siteid = 1) 
    {
    	...
    	if($siteid) 
    	{
			...
			$this->site_setting = Rookie_Cache_Php::get("attachment_config");
    ...
    function thumb($image, $filename = '', $maxwidth = 200, $maxheight = 200, $suffix='', $autocut = 0, $ftp = 0) 
    {
		...
		//处理缩略图处理
		//$filename  = substr($image, 0, strrpos($image, '.')).$suffix.'.thumb.'.$type;
	...
	function watermark($source, $target = '', $w_pos = '', $w_img = '', $w_text = 'phpcms',$w_font = 8, $w_color = '#ff0000') {
		...
		if(!empty($w_img) && file_exists($w_img)) {
			...
			$ifwaterimage = 0;
			$temp = imagettfbbox(ceil($w_font*2.5), 0, SYSPATH.'data/font/elephant.ttf', $w_text);
			$width = $temp[2] - $temp[6];
