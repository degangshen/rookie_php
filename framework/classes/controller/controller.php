...
class controller {
    ...
    public function cache($cacheType='file'){
        return Rookie_Core::cache($cacheType);
    }
	...
	{
		//是否开启运行统计
		if(Rookie_Core::$config['isRunStatic'] === TRUE)
		{
			$content  = '<div><p><b>性能显示：</b><br>运行时间：<font color=red>' . (microtime()-SYS_START_TIME) . '</font>&nbsp;&nbsp;使用内存：<font color=red>' . round(ROOKIE_START_MEMORY / 1048576 * 100) / 100 . ' mb'.'</font><br>';
