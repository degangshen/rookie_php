...
/**
 * DooUriRouter解析服务器的请求的URI
 *
 * URI路由器解析URI，并返回匹配的控制器，动作和参数定义在航线配置。
 * 自动路由处理，如果<b>自动布线</ B>上。控制器可以禁用自动布线的要求，通过写
 * <code>public $autoroute = false;</code>
 * </p>
 *
 * <p>开放路由器进行测试，并应在最现代Web服务器，如工作<b>Apache</b> and <b>Cherokee</b>
 * mod_php，并且或FastCGI模式。
 *
 * <p>可以使用HTTP摘要认证的URI路由器。
 * HTTP摘要更是建议在使用HTTP基本认证不提供任何加密。
 * 如果您正在运行在Apache PHP在CGI/ FastCGI的模式，您将需要
 * 添加下面一行到你的。htaccess消化AUTH才能正常工作。</p>
 * <code>RewriteRule .* - [E=HTTP_AUTHORIZATION:%{HTTP:Authorization},L]</code>
 *
 * <p>几乎相同的路线，可以通过设置每个相同的路线匹配的URI模式的解析。</p>
 ...
 * <code>
 * $route['autoroute_force_dash'] = true;	//设置虚假或不定义将保持路线2的URL访问。
 * </code>
 * </p>
 *
 ...
 * @since 1.0
 */
class Rookie_Uri_Router{

	public static $module = null;
    /**
     * 主要功能被称为以解析请求的URI。
     *
     * <p>返回的参数列表可以作为一个ASSOC数组访问。</p>
     *
     * <code>
    ...
    public function execute($routeArr,$subfolder='/'){
        list($route, $params) = $this->connect($routeArr,$subfolder);
       	...
       	{
        	//自动路由
       		list($control, $method, $method_name_ori, $params) = $this->auto_connect();
       		//如果第三个参数没有
       		if ( ! isset($params[0]))
       			Rookie_Core::system_error();
       			//exit( 'module not null' );
       		
       		//模块名称后面添加了.html扩展名，去掉.html
       		if (strstr($params[0], '.html'))
       			$route['m'] = substr($params[0], 0, strpos($params[0], '.html'));
       		else 
       			$route['m'] = $params[0];
        ...
        if(isset($route['auth'])===true){
            ...
            auth_digest::http_auth($route['authName'],$route['auth'], $route['authFail'], $route['authFailURL']);
        }
        ...
        //返回 Controller class, method, parameters of the route $route['m'] 模块名
        return array($route[0],$route[1],$params,$route['m']);
    }
	...
	private function connect($routes, $subfolder) {
		...
		$end_length = substr_count($requestedUri, '/');
		...
		if ($skipNormalRoutes===false) {
			...
			self::$module = isset($possibleRoutes[$requestedUri]['m']) ? $possibleRoutes[$requestedUri]['m'] : NULL;
			...
			if (isset($possibleRoutes[$requestedUri])===true) {
				...
				if (false === strpos($requestedUri, ':')) {
					...
					$possibleRoutes[$requestedUri][] = self::$module;
